...
```{r}
#3-lag-setup (predicting t+1 with t,t-1,t-2, and t-3)
...
laghistoricalmeanpredictions <- matrix(0, ncol = 49, nrow = (nrow(df)+1))
laglinearpredictions <- matrix(0, ncol = 49, nrow = (nrow(df)+1))
laglassopredictions <- matrix(0, ncol = 49, nrow = (nrow(df)+1))
...
for (tau in windowlength:nrow(df)) {
        ...
        #find historical mean value
        laghistoricalmeanpredictions[tau+1,j] <- mean(train.target)

        #find linear model prediction
        temp.mdl.linear <- lm(train.target ~ ., data=train.full)
        laglinearpredictions[tau+1,j] <- predict(temp.mdl.linear,
                                        newdata= tau.newdata)
        laglinearcoefs[tau+1,] <- coef(temp.mdl.linear)
        ...
        temp.mdl.lasso.mancv <- cv.glmnet(as.matrix(train.predictors),
                               ...
                               alpha = 1, nfolds=5,
                               lambda = c(0.01,0.025,0.05))
        laglassopredictions[tau+1,j] <- predict(temp.mdl.lasso.mancv,
                                        newx = as.matrix(tau.newdata),
                                    type = "response")
        laglassocoefs[tau+1,] <- as.vector(coef(temp.mdl.lasso.mancv))
}
##because this code takes a while to run, print for progress
print(paste(round((tau-80)/444,3)*100,"%",sep=""))
}
