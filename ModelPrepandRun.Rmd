...
```{r alllag1newsmodel}
# set.seed(848)
# historicalmeanpredictions.1.news <- matrix(0, ncol = 49, nrow = (nrow(df.1.news)+1))
# lassopredictionscv.1.all.news <- matrix(0, ncol = 49, nrow = (nrow(df.1.news)+1))
# df <- df.1.news
# 
# #windows run from the start of the window to the last full record
# for (tau in windowlength:nrow(df)) {
...
# print(paste(round((tau-80)/444,3)*100,"%",sep=""))
# }
# save(lassopredictionscv.1.all.news,
...
save(lassopredictions.4.only,
     ...
     file="allpredictions4only.RData")
```
...
lagrfpredictions.4.all.clean <-
as.data.frame(lagrfpredictions.4.all)[(windowlength+1):(nrow(lagrfpredictions.4.all)-1),]
lagrf.4.allSE <- (lagrfpredictions.4.all.clean - lag4actualsdf)^2
lagrf.4.allSSE <- colSums(lagrf.4.allSE)
...
#      file="allpredictions4onlynews.RData")
```
